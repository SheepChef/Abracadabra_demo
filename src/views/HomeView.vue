...
watch(Geeker, () => {
  setTimeout(() => {
    document.getElementById("ImWindowContainer").style.height = "100.1%";
    setTimeout(() => {
      document.getElementById("ImWindowContainer").style.height = "100%";
    }, 500);
  }, 1000);
});
  ...
  try {
    ...
    if (document.getElementById("UseTOTP").checked == true) {
      ...
      var TOTPExpiry = setInterval(function () {
        try {
          if (document.getElementById("TOTPEpoch").value == "") {
            TOTPEpoch.value = Date.now();
          } else {
            TOTPEpoch.value = parseInt(document.getElementById("TOTPEpoch").value);
          }
      ...
      await new Promise((resolve) => {
        Abra.WenyanInput(
          ...
          },
          {
            ...
            TOTPTimeStep: parseInt(
              UseTOTP.value ? document.getElementById("TOTPTimeStep").value : 4
            ),
            TOTPBaseKey: UseTOTP.value
              ? document.getElementById("TOTPBasekey").value == ""
                ? key
                : document.getElementById("TOTPBasekey").value
              : undefined,
...
<template>
    ...
    >
      ...
      <div id="CopyrightBadger" style="grid-area: 4; display: grid; grid-template-columns: 50% 50%">
        <p
          ...
          "
        >
          Abracadabra V3.3.3<br /><a href="https://github.com/SheepChef/Abracadabra">Github Repo</a>
        </p>
        <mdui-chip
          v-if="ShowPWAButton"
          elevated
  ...
  <Card id="AdvancedEncCard" v-if="AdvancedEnc && EncMode == 'Next'">
    <div id="MainContainer">
      ...
      <mdui-text-field
        ...
        variant="outlined"
        rows="1"
        :label="'TOTP Epoch ' + TOTPEpoch"
        placeholder="留空为现在时间戳"
        style="grid-column: span 3; align-self: center; width: 100%; scale: 0.7"
      ></mdui-text-field>
      <mdui-text-field
        v-if="UseTOTP"
        id="TOTPBasekey"
